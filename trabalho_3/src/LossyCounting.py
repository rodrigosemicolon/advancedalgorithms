...
class lossycounting_sd:
    ...
    def get_stats(self) -> None:
        ...
        with open(self.path, "r", encoding="utf-8") as f:
            ...
            self.source_exact_count = literal_eval(
                f.readline().strip("Exact Counts:")[:-1]
            )
    ...
    def get_frequency(self, s: float) -> dict:
        ...
        for ranking in self.ranks:
            ...
            if self.t[item] >= (s - self.epsilon) * self.n:
                ...
                stats_structure[item]["Relative Error"] = (
                    100 * abs(real_freq - est_freq) / real_freq
                )
    ...
    def get_frequency_table(self, s: float) -> str:
        ...
        table = []
        desc = (
            "Lossy Counting implementation with a common shared delta for frequency threshold "
            + str(s * 100)
            + "%"
            + " and epsilon "
        ...
        for item in stats_structure:
            table.append(
                [
                    item,
                    str(stats_structure[item]["Exact Rank"]),
...
class lossycounting_id:
    ...
    def get_frequency_table(self, s: float) -> str:
        ...
        table = []
        desc = (
            "Lossy Counting implementation with individual deltas for frequency threshold "
            + str(s * 100)
            + "%"
            + " and epsilon "
            + str(self.epsilon * 100)
